...
/*
 * @(#)JXBasicComboUI.java	1.155 03/01/23
 *
...
/**
 * Basic UI implementation for JXCombo.
 * <p>
 ...
 */
public class JXBasicComboUI extends ComboBoxUI
{
	...
	// Control the selection behavior of the JXCombo when it is used
	// in the JTable DefaultCellEditor.
	private boolean isTableCellEditor = false;
	private static final String IS_TABLE_CELL_EDITOR = "JXCombo.isTableCellEditor";
	...
	// The implementation of JXComboPopup that is used to show the popup.
	protected JXComboPopup popup;
	...
	// Listeners that are attached to the JXCombo
	/**
	 * This protected field is implementation specific. Do not access directly
	...
	// Listeners that the JXComboPopup produces.
	...
	// Added to the current JXCombo model
	/**
	 * This protected field is implementation specific. Do not access directly
	 * or override. Override the listener construction method instead.
	 *
	 ...
	 *
	 * @return an instance of <code>JXComboPopup</code>
	 * @see JXComboPopup
	 */
	protected JXComboPopup createPopup()
	{
		JXComboPopup popup = new JXComboPopup(comboBox);
	 ...
	 * @see javax.swing.JXCombo#setRenderer
	 */
	 ...
	 * @see javax.swing.JXCombo#setEditor
	 */
	 ...
	 * JXCombo.selectWithKeyChar() so that it can do type-ahead.
	 *
	 * This public inner class should be treated as protected.
	 * Instantiate it only within subclasses of
	 * <code>JXBasicComboUI</code>.
	 */
	public class Handler extends KeyAdapter implements FocusListener,ListDataListener,PropertyChangeListener,LayoutManager
	{
		public void keyPressed(KeyEvent e)
		{
			if(e.getKeyCode() == e.VK_DOWN)
			{
				...
				if(comboBox.isEnabled() && comboBox.isShowing())
				{
					...
					{
						JXBasicComboUI ui = (JXBasicComboUI)comboBox.getUI();
		...
		public void propertyChange(PropertyChangeEvent e)
		{
			...
			else if(propertyName.equals(JXBasicComboUI.IS_TABLE_CELL_EDITOR))
			{
	...
	/**
	 * Determines if the JXCombo is focus traversable.  If the JXCombo is editable
...
return 1;
	...
	protected void selectNextPossibleValue()
	{
		...
		{
			if(isTableCellEditor)
				listBox.changeSelectedRow(si + 1);
			else
	...
	/**
	 * Gets the insets from the JXCombo.
	 */
	...
	public void paintCurrentValue(Graphics g,Rectangle bounds,boolean hasFocus)
	{
		...
		TableCellRenderer renderer = listBox.getColumnEx(listBox.getDisplayIndex()).getCellRenderer();
		Component c;

		if(hasFocus && !isPopupVisible(comboBox))
		{
			c = renderer.getTableCellRendererComponent(listBox,
													   comboBox.getSelectedItem(),
													   true,
													   false,-1,0);
		}
	...
	protected Dimension getDefaultSize()
	{
		...
		TableCellRenderer renderer = listBox.getColumnEx(listBox.getDisplayIndex()).getCellRenderer();
		Dimension d = getSizeForComponent(renderer.getTableCellRendererComponent(listBox," ", false,false,1,0));
	 ...
	 * @see javax.swing.JXCombo#setPrototypeDisplayValue
	 */
	protected Dimension getDisplaySize()
	{
		...
		if(prototypeValue != null)
			result = getSizeForComponent(renderer.getTableCellRendererComponent(listBox,prototypeValue,false,false,0,0));
		else
		{
			...
			if(modelSize > 0)
			{
				...
				{
					d = getSizeForComponent(renderer.getTableCellRendererComponent(listBox,model.getElementAt(i),false,false,-1,0));
	...
	/**
	 * Adds keyboard actions to the JXCombo.  Actions on enter and esc are already
	...
	static class NavigationalAction extends AbstractAction
	{
		...
		{
			JXCombo comboBox = (JXCombo)ev.getSource();
		...
		int getNextIndex(JXCombo comboBox)
		{
			switch(keyCode)
			{
	...
	static class EnterAction extends AbstractAction
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			if(ui.isTableCellEditor())
			{
				...
				comboBox.setSelectedIndex(ui.popup.getList().getSelectedRow());
			}
			else
			{
				if(comboBox.isPopupVisible())
	...
	class EditorActionListener implements ActionListener
	{
		...
		// from the editor may be handled by the JXCombo in which case, the
